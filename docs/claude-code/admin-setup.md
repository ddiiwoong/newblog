...
1. [배포 전략](#1-배포-전략)
2. [공급자와 자격증명](#2-공급자와-자격증명)
3. [Claude apps gateway](#3-claude-apps-gateway)
4. [네트워크와 보안](#4-네트워크와-보안)
5. [거버넌스와 정책](#5-거버넌스와-정책)
6. [모니터링과 비용](#monitoring-cost)
7. [신원, 데이터, 컴플라이언스](#7-신원-데이터-컴플라이언스)
8. [트러블슈팅](#8-트러블슈팅)
...
GPG 서명 검증은 "이 바이너리가 Anthropic이 빌드한 진짜 파일인지" 확인하는 과정입니다. 공급망 공격(supply chain attack)에서 공격자가 패키지 저장소를 변조하거나 미러를 탈취해도, GPG 서명이 일치하지 않으면 설치가 거부됩니다. 지문(`31DD DE24 DDFA B679...`)을 공식 채널과 대조하는 것은 "이 키의 소유자가 Anthropic이 맞는지" 최초 1회 확인하는 절차입니다.
...
| 단계 | 동작 | 잔여 위험 |
| --- | --- | --- |
| **원천 차단** | IdP 계정 비활성 즉시 신규 인증 불가 | — |
| **세션 만료** | SSO 세션과 STS 만료를 업무 리듬에 맞게 단축 (예: 8시간) | 만료 전 남은 시간 |
