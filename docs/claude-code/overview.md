...
| **Agent** | 독립 컨텍스트 서브에이전트 생성, 완료 시 요약 반환 | 자동/명시 |
...
| **탐색과 분리** | 복잡한 문제는 Plan 모드로 조사 먼저, 코딩은 그다음 |
...
> 💡 모든 인터페이스는 동일한 Claude Code 엔진에 연결되므로 CLAUDE.md, settings, MCP 서버가 전부 공유됩니다.
...
> **"읽고, 편집하고, 실행하는 에이전트 — Gather, Act, Verify를 반복하며, 터미널을 넘어 모든 개발 접점에서 동일 엔진으로 동작합니다."**
