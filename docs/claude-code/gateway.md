...
세 가지가 나왔습니다. 첫째, 로그인에는 **문서에 없는 관문 세 개**가 있고 전부 기본값이 거부이거나
대기입니다. 둘째, 그중 하나는 **텔레메트리를 완전히 막는다** — 로그를 보지 않으면 원인을 알 수 없습니다.
셋째, 관문을 통과하면 게이트웨이의 실질적 가치가 나타납니다. **개발자가 지울 수 없는 신원이
모든 텔레메트리에 찍혀 나옵니다.**
...
3. [관문 세 개 — 문서에 없는 부분](#3-관문-세-개--문서에-없는-부분)
...
활성화됐다는 신호입니다. 그리고 이 한 줄이 §3의 세 번째 관문을 유발합니다.
...
빨간 상자 셋이 문서에 없는 관문입니다. 셋 다 기본값이 거부이거나 대기이고, 통과하지 못하면
세션은 겉으로만 살아 있습니다.
...
관문 1을 통과하면 CLI가 디바이스 코드와 검증 URL을 표시하고 폴링에 들어갑니다. 브라우저는
...
세 관문 중 이것만 기본 커서가 1번 Yes에 있습니다. 앞의 둘과 달리 Enter로 승인됩니다.
...
`docker exec -it`로 붙으면 §3의 관문 세 개를 손으로 통과할 수 있습니다. 순서는 이렇습니다.
...
이 실습의 결론은 게이트웨이가 동작한다는 것보다 **개발자 쪽 관문이 문서보다 많다**는 쪽입니다.
