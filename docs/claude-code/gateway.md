...
사용량을 자체 관측 스택으로 받게 하는 게이트웨입니다. 게이트웨이를 세우고 엔드포인트를 하나씩
찔러 보면 무엇을 내려주는지는 알 수 있습니다. 그런데 **CLI가 그걸 어떻게 처리하는지**는 서버 쪽에서
보이지 않습니다.

이 글은 로컬에 IdP와 저장소, 관측 스택까지 세우고 **진짜 Claude Code를 로그인시켜** 확인한
기록입니다.

세 가지가 나왔습니다. 첫째, 로그인에는 **문서에 없는 승인 프롬프트 세 개**가 있고 전부 기본값이 거부이거나
대기입니다. 둘째, 그중 하나는 **텔레메트리를 완전히 막는다** — 로그를 보지 않으면 원인을 알 수 없습니다.
셋째, 승인을 통과하면 게이트웨이의 실질적 가치가 나타납니다. **개발자가 지울 수 없는 신원이
모든 텔레메트리에 찍혀 나옵니다.**
...
1. [서버를 찔러보는 것과 클라이언트를 붙이는 것](#1-서버를-찔러보는-것과-클라이언트를-붙이는-것)
2. [실습 환경 — 컨테이너와 세 가지 제약](#2-실습-환경--컨테이너와-세-가지-제약)
3. [승인 프롬프트 세 개 — 문서에 없는 부분](#3-승인-프롬프트-세-개--문서에-없는-부분)
4. [정책 전달 — 실제 CLI가 받은 것](#4-정책-전달--실제-cli가-받은-것)
5. [텔레메트리 — 신원이 찍혀서 나온다](#5-텔레메트리--신원이-찍혀서-나온다)
6. [대시보드 — 게이트웨이 세션에서만 가능한 질문](#6-대시보드--게이트웨이-세션에서만-가능한-질문)
7. [관측 스택에서 조용히 실패하는 것](#7-관측-스택에서-조용히-실패하는-것)
8. [트러블슈팅](#8-트러블슈팅)
9. [부록: 나머지 설정 파일과 실행 절차](#9-부록-나머지-설정-파일과-실행-절차)
...
게이트웨이 검증에는 두 층이 있습니다. 엔드포인트를 호출해 응답을 보는 것과, 실제 CLI를 연결해
그 응답이 어떻게 쓰이는지 보는 것입니다. 확인 가능한 범위가 다릅니다.
...
앞의 두 항목만 필요하면 `curl`로 충분합니다. 인증 흐름과 정책 응답은 그렇게 검증할 수 있습니다.

문제는 아래 세 항목입니다. **관리자가 설정 한 줄을 바꿨을 때 개발자 화면에 무엇이 뜨는지는
서버 쪽에서 관측되지 않습니다.** 이 글의 발견 대부분이 거기서 나왔습니다.

---

## 2. 실습 환경 — 컨테이너와 세 가지 제약

평소 쓰는 개발 머신에서 그대로 하기는 어렵습니다. 게이트웨이 연결은 **머신 전체에 적용되는
관리 설정 파일**로 켜지기 때문입니다.
...
넘어갑니다. 이미 쓰고 있던 로그인이 끊기고, 실습이 끝나면 되돌려야 합니다. 사용자를 바꿔가며
정책 차등을 보려면 로그인을 반복해야 하는데 그때마다 평소 작업 환경이 흔들립니다.

컨테이너는 이 문제를 없앱니다. 관리 설정 파일과 `~/.claude`가 컨테이너 안에만 있으니 호스트는
무해하고, 상태를 지우고 처음부터 다시 하기도 쉽습니다. 게이트웨이 서버가 **프로덕션에서는
Linux만 지원**(macOS는 로컬 개발용)이라는 점도 컨테이너와 맞습니다.

환경 구성에서 알아야 할 것은 세 가지입니다.

**첫째, 클라이언트와 게이트웨이가 같은 `localhost`를 봐야 합니다.** `http://`는 게이트웨이
호스트가 loopback일 때만 허용되므로, TLS 인증서를 만들지 않으려면 둘이 같은 네트워크
네임스페이스에 있어야 합니다. 텔레메트리 목적지도 같은 제약을 받습니다.
...
수집기를 별도 호스트명으로 두면 이렇게 부팅이 실패합니다. 문서가 제시하는 해법은 HTTPS로
노출하거나 사이드카로 두는 것이고, 후자를 택해 `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1`을 줬습니다.

**둘째, issuer 주소는 브라우저도 해석할 수 있어야 합니다.** 게이트웨이는 디스커버리에서 얻은
`authorization_endpoint`로 브라우저를 리다이렉트합니다. issuer를 `http://keycloak:8081`처럼 도커
네트워크 이름으로 두면 그 주소가 리다이렉트에 실려 나가고, 개발자 브라우저는 해석하지 못해
사인인이 그 지점에서 멈춥니다. Keycloak도 같은 네임스페이스에 두면 issuer를
`http://localhost:8081`로 통일할 수 있다 — 게이트웨이와 브라우저가 같은 주소를 씁니다.

**셋째, 게이트웨이는 IdP보다 먼저 뜰 수 없습니다.** 부팅은 OIDC 디스커버리에 fail-closed다.
...
기동 순서를 보장해야 합니다. 오케스트레이터에서는 initContainer나 readinessProbe가 같은 역할입니다.
...
활성화됐다는 신호입니다. 그리고 이 한 줄이 §3의 세 번째 승인 프롬프트을 유발합니다.
...
세 제약의 귀결입니다. 브라우저는 호스트에서 접속하는데, 게이트웨이가 퍼블리시한 8080·8081을
쓰므로 게이트웨이·IdP 양쪽에 같은 `localhost` 주소로 닿습니다.
...
`gw-server`와 `gw-client`가 같은 이미지를 씁니다. 게이트웨이 서버와 개발자 클라이언트가 같은
바이너리이기 때문입니다. 이미지는 `ubuntu:24.04`에 `claude.ai/install.sh`로 네이티브 설치한
것이다 — npm 설치본은 `requires the native binary`로 게이트웨이가 부팅하지 않습니다.
...
`forcedLoginMethod: "gateway"`가 잡혔습니다. 이 상태에서 비대화식 로그인은 거부됩니다.
...
**게이트웨이 로그인은 대화식 `/login` 전용입니다.** 우회 플래그도, 비대화식 경로도 없습니다.
CI 파이프라인이 게이트웨이로 인증할 수 없는 근본 이유가 이것입니다.

---

## 3. 승인 프롬프트 세 개 — 문서에 없는 부분

문서는 "`/login`을 실행하고 Cloud gateway 화면에서 Enter를 누른 뒤 브라우저 로그인을
완료한다"고만 씁니다. 실제로는 확인 화면이 셋 더 나옵니다.
...
flowchart LR
    ...
    B --> G1["승인 1<br/>게이트웨이 지문<br/>기본 커서 = No"]
...
```mermaid
flowchart LR
    P["폴링 성공"] --> G2["승인 2<br/>작업 디렉터리 신뢰"]
    G2 --> G3["승인 3<br/>관리 설정 승인"]
...
빨간 상자 셋이 문서에 없는 승인 프롬프트입니다. 셋 다 기본값이 거부이거나 대기이고, 통과하지 못하면
세션은 겉으로만 살아 있습니다.
...
TUI가 문자 단위로 커서를 옮겨 그려 캡처에서 공백이 사라진 부분이 있습니다. 원문 그대로입니다.

**기본 커서가 2번 "No, go back"에 있습니다.** 개발자가 명시적으로 올려서 승인해야 합니다.
`http://` loopback 배치에서는 지문 자리에 리터럴 `http-loopback`이 들어갑니다. 핀할 인증서가
없기 때문입니다. 승인 결과는 호스트명별 지문 맵으로 기록됩니다.

```json
{ "gatewayTrust": { "localhost": "http-loopback" } }
```

> **실제 확인**: `claude auth logout`은 세션뿐 아니라 이 핀까지 지웁니다. 사용자를 바꿔가며
> 실습하면 매번 지문 프롬프트를 다시 봅니다. 운영에서는 인증서 교체 시 전 개발자가 이 화면을
> 보게 되므로 계획된 이벤트로 다뤄야 합니다.

### 사이: 브라우저가 하는 일

승인 1을 통과하면 CLI가 디바이스 코드와 검증 URL을 표시하고 폴링에 들어갑니다. 브라우저는
게이트웨이의 검증 페이지를 먼저 만납니다.
...
여기서 "This matches my device"라고 묻는 이유가 있습니다. 디바이스 코드 플로우는 코드를 아는 사람이
승인하는 구조라, 피싱으로 얻은 코드를 남이 승인하게 만드는 공격이 성립합니다. 화면에 뜬 코드가
**내 터미널의 코드와 같은지** 확인하라는 것이 이 문구입니다.

버튼을 누르면 IdP로 리다이렉트되고, 로그인 후 `/oauth/callback`으로 돌아와 사인인이 확정됩니다.
그 시점에 터미널의 폴링이 성공합니다.

리다이렉트 URL에 두 가지가 보입니다. **PKCE(S256)가 기본으로 켜져 있고**(`use_pkce` 기본값 `true`),
...
그리고 이 왕복은 **같은 브라우저에서 끝나야 합니다.** 게이트웨이가 `/device` 응답에 심은 `gw_dev`
쿠키의 해시를 `/oauth/callback`에서 대조합니다. 다른 브라우저에서 시작된 링크는 거부됩니다.
...
피싱으로 얻은 코드를 다른 곳에서 승인시키는 공격을 막는 두 번째 층입니다. 첫 번째 층이 검증
페이지의 "This matches my device" 확인이었습니다.
...
수락하지 않으면 **세션이 준비 상태에 도달하지 못합니다.** 겉으로는 REPL이 떠 있지만 훅도,
텔레메트리도 동작하지 않습니다. 유일한 단서는 디버그 로그입니다.

```
[DEBUG] Skipping SessionEnd:other hook execution - workspace trust not accepted
```

이것 때문에 한동안 "CLI가 텔레메트리를 안 보낸다"고 오진했습니다. `session.count`는 세션 시작에
발생하는 메트릭인데, 세션이 시작되지 않았던 것입니다.

실습을 반복할 때는 `~/.claude.json`에 미리 심어 이 화면을 건너뛸 수 있습니다.
...
승인 전에는 CLI가 원격 설정 로딩 자체를 미룹니다. 그리고 텔레메트리는 원격 설정을 기다립니다.
...
승인이 없으면 원격 설정이 적용되지 않고, 그러면 텔레메트리가 초기화되지 않습니다. 승인 후에야
`~/.claude/remote-settings.json`이 생기고 다음이 이어집니다.
...
> 관리 설정에 자동 주입하고, 그 변수가 승인 목록에 올라갑니다. 즉 **게이트웨이에 텔레메트리를
> 켜는 것은 연결된 모든 개발자에게 승인 프롬프트를 띄우는 일입니다.** 롤아웃 공지 없이 넣으면
> "누가 내 프롬프트를 가로챈다는 경고가 떴다"는 문의가 먼저 옵니다.
...
`apiProvider`가 `firstParty`에서 `gateway`로 바뀐 것이 실제 연결 완료 신호입니다. 자격증명
파일은 두 키를 가집니다. `gatewayTrust`는 지문 맵이고 `enterpriseGateway`는 게이트웨이 세션입니다.

---

## 4. 정책 전달 — 실제 CLI가 받은 것

게이트웨이 설정의 정책 부분은 이렇게 뒀습니다. 그룹별로 다른 것을 내려주고, 마지막 `match: {}`이
인증만 통과한 모두에게 적용되는 catch-all 베이스입니다.
...
이 정책이 실제 CLI에 어떻게 도달하는지 사용자 넷으로 확인했습니다. 그룹 구성을 달리해 정책 해석이
갈리도록 했습니다.
...
**`match.groups`는 부분 겹침으로 매칭됩니다.** alice는 그룹이 둘이고 bob은 하나인데 같은 정책을
받았습니다. `match: { groups: [eng-platform] }`에 **하나만 겹치면** 매칭입니다. 여러 그룹을 나열하면
OR로 동작한다는 뜻이고, AND 조건은 표현할 수 없습니다.

**매칭되는 정책이 없으면 거부가 아니라 catch-all로 떨어집니다.** dan의 `platform-finops`는 어느
정책의 `match.groups`에도 없습니다. 그런데도 세션은 정상 발급되고 catch-all 베이스의
`availableModels`를 받았습니다. **정책에 그룹을 빠뜨리면 조용히 베이스 권한이 부여된다** — 거부를
기대했다면 위험한 기본값입니다. 인증 자체를 막으려면 정책이 아니라 `oidc.allowed_groups`를 씁니다.

**deny는 전원 동일합니다.** 넷 모두 `permissions.deny`가
`["Read(./.env)", "Read(./secrets/**)"]`다. catch-all 베이스의 deny가 **합집합**으로 내려온 것이고,
어느 경로로 매칭돼도 유지됩니다. 역할별 정책이 조직 전체 금지 규칙을 실수로 떨어뜨릴 수 없다는
설계입니다. 반대로 `enforceAvailableModels`는 형제 정책끼리 상속되지 않아 carol만 가집니다.

carol에게 허용 밖 모델을 요청하면 서버가 막습니다. 클라이언트를 패치해도 통과하지 못합니다.
...
alice는 세 모델을 써서 합계 $0.479, carol은 haiku만 쓸 수 있어 $0.031입니다. **16배 차이가
`availableModels` 한 줄에서 나옵니다.** 지출 한도가 사후 차단이라면 모델 allowlist는 사전 억제입니다.
둘은 다른 층에서 동작하므로 함께 씁니다.

dan이 흥미롭습니다. 정책을 명시하지 않았는데 sonnet-4-5를 써서 $0.093을 썼습니다. **의도하지 않은
그룹이 베이스 권한으로 비용을 발생시키는 경로**가 여기 있습니다.

CLI가 캐시한 파일에서도 같은 값이 확인됩니다.
...
갱신은 ETag로 절약됩니다. CLI 디버그 로그에 그대로 찍힙니다.
...
ETag 값은 응답 본문의 `checksum` 필드와 같습니다. 클라이언트는 어느 쪽을 써도 됩니다.
...
> **실제 확인**: 자동 주입되는 env는 6개입니다. 관리자가 쓴 게 아니라 `telemetry.forward_to`를
> 설정했더니 게이트웨이가 넣은 것입니다.
> ```json
...
> ```
> 개발자도 정책도 OTEL을 설정할 필요가 없습니다. 대신 §3의 승인 대화상자가 따라옵니다.

### 업스트림 자격증명 — 명시하지 않으면 조용히 60초를 버린다

Bedrock 업스트림의 `auth`가 받는 필드는 넷입니다. 게이트웨이 바이너리에서 추출한 스키마입니다.
...
`aws_bearer_token`은 Bedrock API 키입니다. SigV4 자격증명도, SSO 로그인도 필요 없어 컨테이너
실습에 가장 잘 맞습니다. 콘솔의 Bedrock → API keys에서 발급하면 `ABSK...` 형태의 문자열이 나옵니다.

문제는 `auth`를 비웠을 때입니다. 환경변수에 키를 넣어도 게이트웨이는 그것을 보지 않고 SigV4 기본
체인만 시도합니다.
...
원인이 자격증명인지 모델인지 알 수 없습니다. 게이트웨이 로그를 봐야 합니다.

> **주의**: 바이너리에 `process.env.AWS_BEARER_TOKEN_BEDROCK`을 읽는 코드가 있어서 환경변수만
> 넣어도 될 것처럼 보입니다. 그건 **CLI가 Bedrock에 직접 붙을 때** 쓰는 경로다 — 같은 문맥에
> `CLAUDE_CODE_SKIP_MANTLE_AUTH`, `X-Amzn-Bedrock-Service-Tier`가 함께 있는 것이 단서입니다.
> 게이트웨이의 업스트림 구성은 별개이고 `upstreams[].auth`만 봅니다.
...
감사 이벤트에 **이메일과 IdP subject가 함께 남는다**는 점이 중요합니다. 추론 한 건이 어느
개발자의 것인지 게이트웨이 로그만으로 특정됩니다. 직접 연결 배포에는 이 층이 없습니다.

SigV4를 쓸 경우 `aws_access_key_id`와 `aws_secret_access_key`는 **반드시 함께** 줘야 합니다.
...
`/v1/models`가 돌려주는 목록은 정책 allowlist와 실제 서빙 가능한 모델의 교집합입니다.
`auto_include_builtin_models: true`로 두면 CLI 내장 카탈로그가 후자를 채웁니다. 그런데 이
카탈로그는 Bedrock보다 늦습니다.
...
내장 카탈로그만 쓰면 Bedrock에 이미 있는 Sonnet 5나 Opus 5를 쓸 수 없습니다. 하지만 그게 상한은
아닙니다. 설정에 `models[]` 배열이 있어 **업스트림 모델 ID를 직접 매핑**할 수 있습니다. 바이너리에서
추출한 스키마입니다.
...
`/v1/models`에 선언 모델과 내장 모델이 함께 나옵니다. 둘의 생김새가 다릅니다.
...
선언한 모델은 `label`이 붙고 ID가 쓴 그대로 유지됩니다. 내장 카탈로그에서 온 모델은 날짜 붙은
ID로 확장되고 `display_name`이 ID와 같습니다. `auto_include_builtin_models`와 충돌하지 않고 공존합니다.

### 선언한 모델이 실제로 동작하는가

실제 CLI 세션에서 `--model`로 지정해 호출했습니다. 셋 다 응답이 왔습니다.
...
`cacheRead`가 모두 0인 것이 정합적입니다. 각 모델의 첫 호출이라 캐시가 없어 `cacheCreation`만
잡혔습니다. 반복 호출한 모델에서는 반대로 `cacheRead`가 커집니다.

비용 열이 의미 있습니다. **Opus 5가 가장 비싼데 토큰은 가장 적게 썼습니다.** Sonnet 5는 토큰이 1.5배인데
비용은 더 낮습니다. 단가 차이가 토큰 수 차이를 덮는다는 뜻이고, `sum by (user_groups)`로만 비용을
보면 원인을 짚을 수 없습니다. §6의 대시보드가 모델별로 쪼개는 이유입니다.
...
두 번째가 함정입니다. alice 정책에는 `claude-opus-4-5`가 있지만 `/v1/models`에는 나오지 않습니다.
이 리전의 해석 단계에서 걸러진 것이고 게이트웨이는 아무 말도 하지 않습니다. **정책에 쓴 모델이
실제로 서빙되는지는 `/v1/models`로 직접 확인해야 합니다.**

날짜 없는 별칭은 날짜 붙은 ID로 해석됩니다. 둘 다 받습니다.
...
한 가지 더. Haiku는 4.5가 최신이고 Bedrock에도 그 위가 없습니다. `models[]`로 선언할 대상 자체가
없다는 뜻입니다. 선언은 **업스트림에 이미 있는 모델을 노출하는 수단**이고, 없는 모델을 만들어
내지는 못합니다.
...
이게 게이트웨이의 실질적 가치입니다. 직접 연결 배포에서는 `OTEL_RESOURCE_ATTRIBUTES`로 개발자마다
신원을 심어야 하고, 그건 개발자가 지울 수 있습니다. 게이트웨이 세션에서는 **CLI가 토큰에서 읽어
스탬프하고, `OTEL_RESOURCE_ATTRIBUTES`로 설정한 `user.*`는 무시됩니다.** 감사 관점에서 전자는
협조 기반이고 후자는 강제입니다.
...
개발자 넷이 각자 세션을 돌린 뒤의 대시보드입니다.

![Claude Code 게이트웨이 세션 대시보드 — 세션 9건, 개발자 4명, 토큰 253K, 추정 비용 $0.7378](/img/cgw-dashboard.png)

읽을 지점이 셋입니다.

**세션 신원 표에 IdP 값이 그대로 있습니다.** 이메일, IdP 그룹, IdP subject가 열로 서 있고
`user.id` 자리에 익명 설치 식별자가 아니라 IdP subject(`6df3e1d0-…`)가 들어갑니다. 사번 대조가
가능하다는 뜻입니다.

**IdP 그룹이 곧 비용 축입니다.** 개발자 × 그룹 × 모델 표에서 `eng-contractors`인 carol은
haiku만 $0.0307을 쓴 반면, `eng-platform`인 alice는 세 모델로 $0.479를 썼습니다. 조직도를 대시보드에
따로 심지 않았는데도 이 분해가 됩니다.

**토큰 총량과 비용 순서가 다릅니다.** 막대그래프는 haiku가 97.6K로 가장 많지만, 파이차트에서
비용 1위는 opus-5($0.3010, 40.8%)다. 단가 차이 때문이고, 그래서 토큰만 보고 비용을 추정할 수 없습니다.

Prometheus까지 오면 신원이 라벨로 승격됩니다.
...
대시보드는 이 라벨이 없으면 물을 수 없는 것만 담았습니다. 패널 13개의 실제 쿼리입니다.
...
모든 쿼리가 `sum(max_over_time(metric[$__range]))` 꼴입니다. 하나만 예외이고(세션 발생 추이),
그 이유는 바로 아래에서 설명합니다. `$__range`는 Grafana가 대시보드 시간 범위로 치환하는
변수라 사용자가 `Last 1 hour`를 고르면 `[1h]`이 됩니다.

템플릿 변수도 IdP 값을 그대로 씁니다.
...
IdP 그룹이 그대로 라벨이 되니 비용 센터별 대시보드가 추가 매핑 없이 만들어집니다. 직접 연결
배포에서는 이 매핑을 조직이 직접 관리해야 합니다.

### 주요 PromQL — 왜 이렇게 써야 하는가

대시보드의 쿼리가 교과서와 다르게 생겼습니다. `rate()`도 `increase()`도 안 씁니다. 데이터 형태가
평범한 서버 메트릭과 다르기 때문입니다.

같은 지표를 네 방식으로 물어본 결과입니다.
...
앞의 셋이 실패하는 이유가 각각 다릅니다.

**인스턴트 쿼리가 빈 결과인 이유.** 프로메테우스 exporter는 비활성 시계열을 만료시킵니다.
세션이 끝나고 몇 분 지나면 `/metrics`에서 사라지고, 인스턴트 쿼리는 볼 것이 없습니다.
...
**`rate()`와 `increase()`가 0인 이유.** `session.id`가 라벨이라 세션마다 새 계열이 생깁니다.
각 계열의 수명은 세션 길이만큼입니다.
...
10초 스크레이프 간격에 표본 30개면 계열이 약 5분 살았다는 뜻입니다. `rate()`는 창 안에서 같은
계열의 **증분**을 보는데, 계열이 나타났다 사라지는 것을 리셋으로 취급합니다. 짧은 계열이 흩어져
있으면 증분을 잡을 구간이 사실상 없습니다.
...
계열별로 시간 범위 안의 **최댓값**을 구한 뒤 합산합니다. 각 계열은 살아 있는 동안 단조 증가하는
카운터이므로 최댓값이 곧 그 세션의 최종값입니다. 계열이 사라져도 TSDB에 표본이 남아 있으니
범위 안이면 잡힙니다. `$__range`는 Grafana가 대시보드 시간 범위로 치환하는 변수라, 사용자가
`Last 3 hours`를 고르면 `[3h]`가 됩니다.

> **주의**: 이 방식은 **같은 세션이 범위를 걸쳐 있으면 과소 집계**됩니다. 범위 시작 이전에 이미
> 쌓인 분량이 최댓값에 포함되기 때문입니다. 세션이 짧은 개발 도구 텔레메트리에서는 문제가 되지
> 않지만, 장기 실행 프로세스 메트릭에 그대로 옮기면 안 됩니다.
...
셉니다. `sum(claude_code_session_count_total)`을 쓰면 안 된다 — 그건 세션 시작 카운터의 합이라
재개(`start_type=resume`)를 별개로 세거나 같은 세션을 중복 계산할 수 있습니다.
...
이 라벨은 게이트웨이 세션에만 붙습니다. 직접 API 키로 붙은 세션에는 `identity_source`도
`user_email`도 `user_groups`도 없습니다. **한 조직에 두 경로가 섞여 있으면 이 필터가 분리 기준이
됩니다.** 필터를 빼면 신원 없는 계열이 합산에 끼어 인당 집계가 흐려집니다.

#### 귀속 축 바꾸기

같은 지표를 `sum by`의 라벨만 바꿔 다른 질문에 답합니다.
...
`user_groups`는 IdP 그룹이 콤마로 이어진 **단일 문자열**입니다. `eng-platform,platform-finops`가
하나의 라벨 값이므로 그룹 하나로 쪼개 집계할 수 없습니다. 비용 센터를 그룹 하나에 대응시키려면
IdP 쪽에서 사용자를 한 그룹에만 넣거나, 수집기 단계에서 라벨을 분리해야 합니다.

`type`을 빼면 안 되는 이유는 실측으로 분명합니다.
...
토큰 총량은 1.8배 차이인데 비용은 5.4배입니다. 단가가 다르기 때문이고, 그래서 **토큰만 보고
비용을 추정할 수 없습니다.** 그리고 총량의 대부분은 `cacheCreation`이다 — `type`으로 쪼개지 않으면
모든 세션의 첫 요청이 과대하게 보입니다.
...
Grafana의 다중 선택 변수는 정규식으로 치환됩니다. `All`을 고르면 `.*`이 되므로 `=~`를 써야 합니다.
`=`를 쓰면 `All`에서 아무것도 안 나옵니다. 변수 값 목록은 라벨에서 직접 뽑습니다.
...
IdP 그룹이 그대로 라벨이므로 조직도를 대시보드에 따로 심을 필요가 없습니다.

#### 카디널리티 주의

`session_id`가 라벨이라 **세션마다 새 계열이 생깁니다.** 실습에서는 5세션 × 모델 × type 조합으로
20계열이었지만, 개발자 수백 명 규모에서는 곧 폭발합니다. 필요 없으면 끕니다.

```
OTEL_METRICS_INCLUDE_SESSION_ID=false
```

끄면 세션 단위 구분은 잃지만 인당·모델별 집계는 그대로 됩니다. `session.id`는 이벤트(로그)에도
있으니 세션 추적은 Loki 쪽에서 하는 편이 낫습니다.

> **주의**: `claude_code.cost.usage`는 추정치이고 게이트웨이의 `spend_limits`와 **별개 계량**입니다.
> 대시보드의 비용과 게이트웨이가 429로 차단하는 한도는 다른 숫자입니다. 공식 청구는 Bedrock을 봅니다.

`token.usage`와 `cost.usage`는 추론이 성공해야 나타납니다. 업스트림 자격증명이 없으면 세션 계열만
쌓입니다. 반대로 말하면 **인증·정책·신원 스탬프는 비용 없이 전부 검증됩니다.**

실제 CLI 세션에서 한 줄 프롬프트를 보낸 결과는 이렇습니다.
...
`cacheCreation`이 23,994로 압도적입니다. 프롬프트는 한 줄인데 시스템 프롬프트와 도구 정의가
프롬프트 캐시에 올라간 것이고, 비용 $0.0303의 대부분이 여기서 나왔습니다. 두 번째 요청부터는
`cacheRead`로 넘어가 훨씬 싸집니다.

> **주의**: 인당 비용을 볼 때 `type`을 분리하지 않으면 **모든 개발자의 첫 요청이 과대하게
> 보입니다.** 대시보드가 `sum by (model, type)`으로 쪼개는 이유입니다. 캐시 생성분을 세션 시작
> 고정비로 따로 보고, 실사용 추이는 input/output으로 판단하는 편이 맞습니다.

---

## 7. 관측 스택에서 조용히 실패하는 것

수집기 설정 전체는 이렇습니다. 짧지만 두 항목이 결정적입니다.
...
Prometheus는 사이드카를 `gw-server` 이름으로 스크레이프합니다. netns를 공유하므로 8889 포트가
gw-server의 주소에 노출됩니다.
...
이제 조용히 실패하는 두 지점입니다. 둘 다 에러 없이 잘못된 결과가 나옵니다.

**첫째, delta를 cumulative로 바꿔야 합니다.** Claude Code의 메트릭 temporality 기본값은 delta인데
Prometheus는 cumulative 카운터를 기대합니다. 변환하지 않으면 카운터가 매 export마다 리셋된 것처럼
보이고 `rate()`와 `increase()`가 엉뚱한 값을 냅니다. 경고는 없습니다.
...
있지만 개발자 환경에 의존합니다. 수집기에서 흡수하는 편이 안전합니다.

**둘째, 리소스 속성을 라벨로 승격시켜야 합니다.** 이게 없으면 `user_email`, `user_groups`,
`identity_source`가 라벨로 올라오지 않아 §6의 질문을 아예 할 수 없습니다. 메트릭은 정상적으로
보이는데 인당 집계만 불가능해집니다.
...
> **주의**: `session_id`가 라벨에 포함되므로 **세션마다 새 시계열이 생깁니다.** 실습에서는 편하지만
> 규모가 커지면 카디널리티가 폭발합니다. `OTEL_METRICS_INCLUDE_SESSION_ID=false`로 끌 수 있습니다.

프로메테우스 exporter가 비활성 시계열을 만료시킨다는 점도 알아둘 만합니다. 세션이 끝나고 몇 분
지나면 `/metrics`가 비고 인스턴트 쿼리도 빕니다. `max_over_time(...[45m])`으로 확인합니다.

### 릴레이 경로를 먼저 검증한다

CLI를 붙이기 전에 합성 OTLP로 게이트웨이 → 수집기 경로만 확인해 두면, 나중에 문제가 생겼을 때
범위를 좁힐 수 있습니다. 실제로 이 검증이 있었기에 "CLI가 안 보낸다"를 빨리 확정할 수 있었습니다.
...
CLI는 delta(1)로 보내므로 §7 앞부분의 변환이 필요합니다.

게이트웨이가 노출하는 수신 경로는 셋이고 모두 200을 반환합니다.
...
`traces: false`인데 200인 것은 의도된 동작입니다. 목적지가 없는 시그널은 받아서 폐기합니다.
...
| 대시보드 패널이 "No data", `rate()`가 0 | 세션마다 계열이 생겼다 만료됩니다. `rate()`·`increase()` 대신 `sum(max_over_time(metric[$__range]))` (§6) |
| 호스트에서 `/readyz` 503인데 컨테이너 내부는 `ready` | 게이트웨이가 `127.0.0.1`에 바인딩됐거나, 앞서 띄운 게이트웨이 프로세스가 포트를 점유. `lsof -nP -iTCP:8080` |
| 컨테이너 재생성 때마다 로그인 소실 | `~/.claude`가 컨테이너 파일시스템에 있습니다. 볼륨으로 분리 |
...
§2의 compose가 마운트하는 파일 중 앞에서 다루지 않은 것들입니다. 이 셋을 채우면 재현이 완결됩니다.
...
§4에 정책·텔레메트리를 실었으니 앞부분만 채우면 됩니다. `store`가 빠지면 부팅하지 않는다 —
디바이스 코드 랑데부가 여기 있어서 사인인 자체가 성립하지 않습니다.
...
문자열이 안 맞습니다. 그룹 매칭은 대소문자 구분 정확 비교입니다.

`realmRoles`를 빼면 realm 임포트 사용자에게 기본 역할이 붙지 않아 게이트웨이의 `offline_access`
스코프 요청이 거부됩니다. 증상이 사인인 **마지막 단계**에 나타나 원인을 찾기 어렵습니다.
...
`dave@outside.test`는 `allowed_email_domains` 거부를 확인하는 용도입니다. Keycloak에서는 정상
인증되지만 게이트웨이가 세션 발급을 거부합니다.

```
{"evt":"session.mint","result":"fail","err":"email domain not allowed"}
```

거부가 **IdP 로그인을 통과한 뒤** 게이트웨이에서 일어난다는 점이 중요합니다. 다중 테넌트 IdP
오설정에 대한 방어선입니다.

### Grafana 프로비저닝

대시보드 JSON이 `uid`로 데이터소스를 참조하므로 고정 `uid`를 줘야 합니다.
...
§6의 표가 각 패널의 쿼리이므로, 그 쿼리를 패널로 옮기면 대시보드가 됩니다.
...
`docker exec -it`로 붙으면 §3의 승인 프롬프트 세 개를 손으로 통과할 수 있습니다. 순서는 이렇습니다.

| 단계 | 화면 | 조작 |
|---|---|---|
| 1 | REPL 하단에 `Not logged in · Run /login` | `/login` 입력 |
| 2 | `Cloud gateway` + URL 박스 | Enter |
| 3 | 승인 1 — `Trust gateway localhost?` + 지문 | 위 방향키로 1번 선택 후 Enter |
| 4 | 디바이스 코드 `XXXX-XXXX` + 검증 URL | 호스트 브라우저로 이동 |
| 5 | 브라우저 — 게이트웨이 검증 페이지 | `This matches my device — continue` |
| 6 | 브라우저 — IdP 로그인 | `carol` / `carol-pw` |
| 7 | 터미널 — 폴링 성공, 승인 2 폴더 신뢰 | 위 방향키로 1번 선택 후 Enter |
| 8 | 승인 3 — 관리 설정 승인 | Enter (기본 커서가 Yes) |

3번과 7번에서 숫자 키는 먹지 않습니다. 방향키로 옮겨야 합니다. 8번만 기본 커서가 Yes라 Enter로
승인됩니다. 4번 URL이 잘려 보이면 코드를 직접 붙여도 됩니다.
...
`bob`과 `dan`이 §4의 매칭 규칙 두 가지를 드러냅니다. 실습에서 이 둘을 빼면 정책이 의도대로
동작하는지 확인할 수 없습니다.

로그인 후 세션 안에서 `/model`을 열면 §4 표의 `availableModels`가 그대로 목록에 반영된 것을
볼 수 있습니다. carol은 haiku 하나뿐입니다.
...
이 실습의 결론은 게이트웨이가 동작한다는 것보다 **개발자 쪽 승인 단계가 문서보다 많다**는 쪽입니다.
지문 신뢰, 작업 디렉터리 신뢰, 관리 설정 승인 — 셋 다 기본값이 거부이거나 대기이고, 통과하지
못하면 세션은 겉으로만 살아 있습니다. 특히 세 번째가 텔레메트리를 막는 경로는 로그를 보지 않으면
알 수 없습니다.
...
유발한다**는 점입니다. `telemetry.forward_to` 한 줄이 `OTEL_EXPORTER_OTLP_ENDPOINT` 자동 주입으로
이어지고 그 변수가 승인 대상입니다. 롤아웃 공지 없이 넣으면 경고 화면을 개발자들이 먼저 봅니다.
다만 이 프롬프트는 정직합니다. 승인 대상은 텔레메트리를 켜는 것이 아니라 데이터가 어디로
가는지입니다.

관측 측면에서 게이트웨이가 주는 것은 명확합니다. `identity.source=gateway-oidc`, `user.email`,
`user.groups`가 CLI에서 자동으로 찍혀 나오고 개발자가 지울 수 없습니다. 직접 연결 배포에서
`OTEL_RESOURCE_ATTRIBUTES`로 신원을 심는 방식은 협조에 기대지만 게이트웨이 세션은 강제합니다.
IdP 그룹이 그대로 라벨이 되니 비용 센터별 대시보드가 추가 매핑 없이 만들어집니다.

한편 관측 파이프라인에서 조용히 틀리는 지점 둘도 기억할 만합니다. delta 카운터를 변환하지
않으면 숫자가 그럴듯하게 틀리고, 리소스 속성을 승격시키지 않으면 게이트웨이가 애써 붙인
신원이 쿼리에서 사라집니다. 둘 다 에러를 내지 않습니다.
...
비교하는 것입니다. 그리고 `logs: true`로 흘러드는 이벤트에 `OTEL_LOG_TOOL_DETAILS=1`을 얹으면
