...
- **장애 복구** — 노드 한 대가 죽으면 감지하고 교체한 뒤 체크포인트에서 재개
...
- **자동 헬스 체크·노드 교체** — 죽은 노드를 감지해 교체하고 마지막 체크포인트에서 재개(장기 학습의 핵심)
...
# 직후
squeue: JOB 2  ST=PD  (BeginTime)   <- 돌던 잡이 죽지 않고 재큐됨
sinfo:  compute-dy-c5n9xl-2  idle%   <- 실패 노드 슬롯 리셋(다음 실행 때 새 인스턴스로 교체)
```

핵심은 세 가지다. ① 헬스 체크가 죽은 인스턴스를 **자동 감지**했고, ② `--requeue` 덕에 돌던 잡이 **조용히 실패하지 않고 다시 큐로 돌아갔으며**, ③ 실패한 노드 슬롯이 정리돼 다음 실행 때 **새 인스턴스로 교체**된다. 체크포인트가 있었다면 학습은 마지막 지점에서 이어진다. 이게 수일짜리 학습을 노드 장애로부터 지켜주는 메커니즘이다.
...
베이스 AMI에 박힌 FSx Lustre apt 레포가 403/서명 만료라 `apt-get update`가 죽고, 그 위에서 도는 라이프사이클이 통째로 실패한 것이다(이 클러스터는 FSx를 안 쓰는데도). `on_create.sh`에서 `lifecycle_script.py`를 부르기 전에 그 깨진 레포를 지우는 한 줄을 넣어 해결했다.
...
네트워크로 노드를 잇고, 스토리지로 데이터를 먹이고, 관측으로 병목을 찾고, 오케스트레이션으로 묶으면 — 분산 학습 인프라의 큰 그림이 완성된다.
...
- [Amazon EKS — Machine Learning](https://docs.aws.amazon.com/eks/latest/userguide/machine-learning-on-eks.html)
...
- [eksctl — EFA 지원 노드그룹](https://eksctl.io/usage/eks-efa/)
