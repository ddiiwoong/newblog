...
> 실측 예시: 아래 [데이터 병목 재현](#data-bottleneck-workers)에서 `dcgmi dmon`으로 GPU 활용률과 SM active를 초 단위로 찍어봤다. 데이터 공급이 밀리면 GPU 활용률이 0 근처까지 떨어졌다가 worker를 늘리면 다시 올라오는데, 그 변화가 초 단위로 그대로 드러난다.
