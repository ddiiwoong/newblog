...
먼저 알아둘 점은, EFA가 모든 사이즈에서 지원되는 것이 아니라는 것이다. EFA는 인스턴스가 호스트의 네트워크 대역폭을 충분히 확보해야 하므로, 많은 인스턴스 계열에서 **가장 큰 사이즈(16xlarge / 32xlarge / 48xlarge 등)** 위주로 지원된다.

- HPC용: `hpc6a`, `hpc7g`, `c5n.18xlarge` 등
- GPU 분산 학습용: `p4d`, `p5`, `g5`, `g6` 등

다만 예외가 있다. 2021년에 AWS가 C5n, G4, I3 등 일부 계열의 작은 사이즈에도 EFA를 확장했다. 예를 들어 `c5n.9xlarge`(절반 사이즈)나 `g4dn.8xlarge`도 EFA를 지원한다. 단순히 "EFA가 동작하는지" 학습/검증하는 용도라면 이런 작은 사이즈가 진입 장벽이 낮아 유용하다.
...
참고로 AWS의 **NCCL + EFA 공식 가이드(getting started)는 가속 컴퓨팅 P 계열 인스턴스만 지원 대상으로 명시**하고 있다. 즉 g5/g6에서도 EFA로 NCCL을 돌려볼 수는 있지만, GPUDirect RDMA가 적용되는 정식 경로에서 EFA의 실제 성능(InfiniBand 대체)을 제대로 검증하려면 `p4d`/`p5` 계열을 사용하는 것이 맞다. g5/g6는 "EFA 멀티노드 통신이 동작하는지" 정도를 가볍게 확인하는 용도로 보면 된다.
