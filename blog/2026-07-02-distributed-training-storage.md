...
* **POSIX 호환**: PyTorch/TensorFlow의 데이터 로더는 기본적으로 파일시스템 경로를 전제한다.
...
* **임시 작업에는 과하다**: 짧은 실험(몇 시간)에 FSx를 띄웠다 내리면 생성·삭제 시간이 더 든다.
...
* **활용**: 데이터 로딩보다는 **체크포인트 저장, 설정 파일 공유, 소규모 학습에** 더 어울린다.
...
* **PyTorch DataLoader와 호환**: 마운트 경로를 그냥 데이터 디렉터리로 지정하면 된다.
...
즉 정확한 결론은 *"병렬 클라이언트가 있어야 프로비저닝한 처리량이 실현되고, 버스트·프로비저닝 한도 안에서 노드 수에 비례해 오른다"까지*다. "노드 N대면 성능이 자동 보장"이 아니라, **프로비저닝으로 상한을 사고, 병렬 클라이언트로 그 상한을 채우는** 구조로 이해하는 게 맞다.
...
먼저 ImageFolder를 tar 샤드로 묶는다. `webdataset`의 `ShardWriter`가 샘플당 `jpg`+`cls`를 한 tar에 쌍으로 담아준다.
...
대형 모델은 FSDP(Fully Sharded Data Parallel)나 DeepSpeed ZeRO로 파라미터를 샤딩한다. 이 경우 각 노드가 자기 샤드만 독립적으로 저장하므로, 파일시스템에 걸리는 동시 쓰기 부하가 분산된다. FSx의 병렬 I/O가 유리한 지점이다.
